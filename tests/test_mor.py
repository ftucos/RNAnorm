...
from rnanorm import MOR
from rnanorm.datasets import load_gtex


@pytest.fixture
def expected_mor(exp):
    return pd.DataFrame(
        [
...
def test_mor(exp, expected_factors, expected_mor):
    ...
    np.testing.assert_allclose(transformed_data, expected_mor, rtol=1e-4)
    ...
    np.testing.assert_allclose(
        transformed_data,
        expected_mor.loc[["Sample_2"]],
        rtol=1e-3,
    )


def test_mor_rnanorm_edger():
    """Test our results against DESeq2's MOR implementation."""
    # Load saved scaling factors from DESeq2
    files_dir = Path(__file__).parent / "files"
    deseq2_factors = pd.read_csv(
        files_dir / "gtex.lung.30.chr21.mor_factors_deseq2.tsv",
        sep="\t",
        comment="#",
        index_col=0,
        usecols=[0, 1, 2],
    ...
    ds = load_gtex()
    rnanorm_factors = MOR().fit(ds.exp).get_norm_factors(ds.exp)

    # Compare loaded and computed scaling factors
    np.testing.assert_array_almost_equal(
        deseq2_factors["norm.factors"].values,
        rnanorm_factors,
        decimal=14,
    )
