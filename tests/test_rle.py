...
def test_rle(exp, expected_factors, expected_rle):
    ...
    np.testing.assert_allclose(transformed_data, expected_rle, rtol=1e-4)
    ...
    np.testing.assert_allclose(
        transformed_data,
        expected_rle.loc[["Sample_2"]],
        rtol=1e-3,
    )


def test_rle_rnanorm_edger():
    """Test our results against EdgeR's RLE implementation."""
    # Load saved scaling factors from edgeR
    files_dir = Path(__file__).parent / "files"
    edger_factors = pd.read_csv(
        files_dir / "gtex.lung.30.chr21.rle_factors_edgeR.tsv",
        sep="\t",
        comment="#",
        index_col=0,
        usecols=[0, 2, 3],
    ...
    ds = load_gtex()
    rnanorm_factors = RLE().fit(ds.exp).get_norm_factors(ds.exp)
